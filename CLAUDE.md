...
**Status: Phases 0–3 complete and tested. Phase 4 (vector RAG) is next.**

## Commands

```powershell
# Tests — 81 of them, ~6 seconds. Run these before and after any change.
python -m pytest
...
app/
  ...
  llm/
    ...
    factory.py          explicit singleton + reset_client()
  ...
  memory/
    manager.py          watermark logic; composes summary + lorebook
...
- **The DB migration helper is additive-only** (`_ADDED_COLUMNS` in `db/database.py`). It
  ...
  rename, that needs a real migration story — probably Alembic.
...
## Phase 4 — vector RAG (next)

The goal is precise long-range recall: "what was the innkeeper's name in chapter one". That's
what summarization is worst at, by nature.

Where it plugs in: `MemoryManager.build_context()` already composes summary + lorebook into a
token budget. Retrieval becomes a third source beside them, not a replacement for either.

Suggested shape:
- Embedding model via Ollama (`/api/embeddings`) so no new runtime dependency.
- `sqlite-vec` keeps everything in the existing SQLite file; Chroma is the batteries-included
  alternative.
- Embed each message on persist (in `orchestrator.stream_reply`, alongside the existing
  `Message` insert), and lorebook entries on card save.
- Retrieve top-k against the recent turns, dedupe against what's already verbatim in context,
  and add a `lore`-style slot in `build_prompt`.

Read `docs/architecture.md` §5 first — it lays out RAG's tradeoffs honestly. Its failure
modes differ in kind from what's already handled: retrieval misfires produce *plausible but
irrelevant* context, which is harder to notice than a missing fact. Budget time for tuning
chunk size, k, and similarity thresholds rather than assuming it works once wired up.

Add tests to `tests/test_memory.py` or a new `test_retrieval.py`. Keep using a mock embedder
...
- **The stack has never run against a real model.** Every test uses `MockLLM`. Prompt format,
  ...
  output.
- **No Unraid install has been performed.** The templates match Unraid's schema but have not
  been imported into a live Docker tab.
